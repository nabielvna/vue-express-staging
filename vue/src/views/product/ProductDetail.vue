            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div class="space-y-6">
                    ...
                    <div class="space-y-4">
                        <button
                            @click="addToCart"
                            class="w-full bg-gray-700 text-white py-3 hover:bg-gray-600 transition-colors relative"
...
// Define emits
const emit = defineEmits(['update-cart-state'])
...
const fetchProduct = async () => {
    ...
    try {
        ...
        if (result.status === 'success') {
            ...
            console.log('Sizes:', product.value.Sizes)
...
const addToCart = async () => {
    if (!selectedSize.value || isAddingToCart.value) return

    try {
        isAddingToCart.value = true

        // Find the size object with the selected size
        const sizeObj = product.value.Sizes.find(
            s => s.size === selectedSize.value,
        )

        if (!sizeObj || !sizeObj.ProductSize) {
            throw new Error('Selected size not found')
        }

        // Get the product size id from the through table (ProductSize)
        const productSizeId = sizeObj.ProductSize.id

        // For debugging
        console.log('Selected Size:', selectedSize.value)
        console.log('Size Object:', sizeObj)
        console.log('ProductSize:', sizeObj.ProductSize)
        console.log('Product Size ID:', productSizeId)

        // Check stock
        if (sizeObj.ProductSize.stock <= 0) {
            error.value = 'This size is out of stock'
            return
        }
...
// Handle ShopPay and other payment options
// const handleShopPay = () => {
//     if (!selectedSize.value) {
//         error.value = 'Please select a size first'
//         return
//     }
//     // Implement ShopPay logic here
// }
...
//         return
//     }
//     // Implement more payment options logic here
// }
